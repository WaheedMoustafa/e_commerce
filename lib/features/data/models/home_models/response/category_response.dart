...
class CategoryResponse {
  ...
  CategoryResponse.fromJson(dynamic json) {
    ...
    if (json['data'] != null) {
      _data = [];
      json['data'].forEach((v) {
        _data?.add(CategoryDM.fromJson(v));
      });
    }
  }
  num? _results;
  Metadata? _metadata;
  List<CategoryDM>? _data;
CategoryResponse copyWith({  num? results,
  Metadata? metadata,
  List<CategoryDM>? data,
}) => CategoryResponse(  results: results ?? _results,
  metadata: metadata ?? _metadata,
  data: data ?? _data,
);
  num? get results => _results;
  Metadata? get metadata => _metadata;
  List<CategoryDM>? get data => _data;
